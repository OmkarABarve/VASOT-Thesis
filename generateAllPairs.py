            ...
            for i in range(0, T - window_size + 1, stride):
                ...
                for j in range(j_start, j_end):
                    ...
                    pairs.append({
                        'window1': w1_feat,
                        'window2': w2_feat,
                        #'window1_labels': w1_lbl, Labels not used in self-supervised learning
                        #'window2_labels': w2_lbl, Labels not used in self-supervised learning
...
def action_consistency_pairs(
    ...
    for video in self.dataset:
        ...
        for i in range(1, T + 1):
            ...
            if end_of_sequence or label_changed:
                segment_end = i  # exclusive
                seg_len = segment_end - segment_start

                if seg_len >= window_size:
                    seg_feat = features[segment_start:segment_end]
                    # build windows inside segment
                    for j in range(0, seg_len - window_size + 1, step):
                        for k in range(j + step, seg_len - window_size + 1, step):
                            pairs.append({
                                'anchor': seg_feat[j:j + window_size],
                                'positive': seg_feat[k:k + window_size],
                                'action_type': current_action,  # <-- USES GROUND TRUTH!
                                'video_id': video['video_id'],
                                'segment_start': segment_start,
                                'segment_end': segment_end,
                                'anchor_start': segment_start + j,
                                'positive_start': segment_start + k,
                            })

                # start next segment if not at end
                if not end_of_sequence:
                    segment_start = i
                    current_action = int(labels_arr[i])  # <-- USES GROUND TRUTH!
